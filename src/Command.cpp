...
bool Command::execute(Area& area) {
    ...
    if (name == "point") {
        ...
        std::cerr << "Usage: rectangle x y width height" << std::endl;
    }

    else if (name == "polygon") {
        if (intParams.size() >= 6) {
            auto poly = std::make_shared<Polygon>(intParams);
            area.addShape(poly);
            std::cout << "Polygone ajouté (ID: " << poly->getId() << ")" << std::endl;
            return true;
        }
        std::cerr << "Usage: polygon x1 y1 x2 y2 x3 y3 ..." << std::endl;
    }

    else {
        std::cerr << "Commande inconnue: " << name << std::endl;
    }
